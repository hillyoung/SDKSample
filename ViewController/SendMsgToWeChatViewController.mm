...
//    }];
    ...
    launchMiniProgramReq.userName = @"gh_fc82fe8a0b1f";  //拉起的小程序的username
    launchMiniProgramReq.path = @"pages/index/index";    ////拉起小程序页面的可带参路径，不填默认拉起小程序首页，对于小游戏，可以只传入 query 部分，来实现传参效果，如：传入 "?foo=bar"。
...
- (void)offlinePay
{
    WXOfflinePayReq *req = [[WXOfflinePayReq alloc] init];
    [WXApi sendReq:req completion:nil];
}

- (void)subscribeMiniProgramMsg
{
//    [UIAlertView requestWithTitle:@"请输入小程序appid" message:nil defaultText:@"gh_fc82fe8a0b1f" sure:^(UIAlertView *alertView, NSString *text) {
//        
//        NSString *miniProgramAppid = text;
//        
//        WXSubscribeMiniProgramMsgReq *req = [[WXSubscribeMiniProgramMsgReq alloc] init];
...
- (void)setupSceneView {
    ...
    [btn_x setFrame:CGRectMake(10, 50, 50, 40)];
    ...
    [btn_y setFrame:CGRectMake(70, 50, 60, 40)];
    ...
    [btn_z setFrame:CGRectMake(140, 50, 50, 40)];
...
- (void)setupFootView {
    ...
    ADDBUTTON_AUTORELEASE(index++, @"离线支付", @selector(offlinePay));
    ADDBUTTON_AUTORELEASE(index++, @"订阅小程序消息", @selector(subscribeMiniProgramMsg));
    
    self.footView.contentSize = CGSizeMake(SCREEN_WIDTH, 30*(index/2) + TOPMARGIN*(index/2));
}
